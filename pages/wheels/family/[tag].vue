  ...
  <div>
    ...
    <div class="max-w-7xl mx-auto px-6 py-10">
      ...
      <div v-else-if="error" class="rounded-lg border border-red-200 bg-red-50 p-6 text-sm">
        <p class="font-semibold text-red-700 mb-1">Could not load product family</p>
        <p class="text-red-600">{{ error.message }}</p>
        <p class="text-red-500 mt-2 text-xs">
          Check that the family tag <code class="font-mono bg-red-100 px-1 rounded">{{ route.params.tag }}</code>
          exists in Odoo and at least one product is published.
        </p>
      </div>
...
const { fetchProductFamily } = useProducts()

const tag = computed(() => route.params.tag as string)

const { data, pending, error } = await useAsyncData<Product>(
  `family-${tag.value}`,
  () => fetchProductFamily(tag.value),
  { watch: [siteContext] },
)
